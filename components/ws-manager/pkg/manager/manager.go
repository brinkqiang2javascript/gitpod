...
import (
	...
	"github.com/gitpod-io/gitpod/ws-manager/pkg/manager/internal/state"
...
func New(ctx context.Context, config Configuration, clientset kubernetes.Interface, cp *layer.Provider) (*Manager, error) {
	...
	// TODO: review resync period. Forcing a full resync can impact the performance.
	kubernetesState := state.NewStateHolder(config.Namespace, workspaceObjectListOptions(), 0, clientset)
