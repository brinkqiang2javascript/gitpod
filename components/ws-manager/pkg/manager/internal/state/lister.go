...
// ByKey returns the Secret matching key in the Secret Store.
func (pl *secretLister) ByKey(key string) (*apiv1.Secret, error) {
	s, exists, err := pl.GetByKey(key)
	if err != nil {
		return nil, err
	}
	...
	if !exists {
		return nil, &k8serr.StatusError{ErrStatus: metav1.Status{
			Code:    http.StatusNotFound,
			Message: fmt.Sprintf("cannot find secret %s", key),
		}}
	}
